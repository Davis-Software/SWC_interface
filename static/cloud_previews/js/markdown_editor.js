...
async function update_md(text){
    ...
    if(!text){
        text = (await (await fetch(location.href + "&raw=true")).json()).data[2]
    }
    preview.innerHTML = text
    hljs.highlightAll()
    preview.querySelectorAll("*[href]").forEach(elem => {
